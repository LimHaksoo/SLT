...
# ---------------------------------------------------------------------
# Attention override: always use PyTorch SDPA (scaled_dot_product_attention)
# to unlock FlashAttention / memory-efficient attention kernels when available.
# This intentionally overrides any timm.Attention import above.
# ---------------------------------------------------------------------

class Attention(nn.Module):
    """Multi-head self-attention using torch.nn.functional.scaled_dot_product_attention.

    This enables PyTorch 2.x to dispatch to FlashAttention / mem-efficient kernels
    when supported by the GPU/driver/PyTorch build and input shapes/dtypes.
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Ensure contiguous for fast kernels:
        q = q.contiguous()
        k = k.contiguous()
        v = v.contiguous()

        # SDPA: will choose flash/mem-efficient/math backend automatically.
        out = F.scaled_dot_product_attention(
            q, k, v,
            attn_mask=None,
            dropout_p=self.attn_drop if self.training else 0.0,
...
class DiTTRM(nn.Module):
    ...
    def __init__(
        ...
        **block_kwargs,
        ...
        if self.inject_x:
            ...
            # If we don't inject x, keep the scalar as a buffer so DDP doesn't expect grads.
            self.register_buffer("inject_scale_x", torch.tensor(float(x_scale0)))
        if self.trm_mode == "self_refine" and self.inject_y:
            self.inject_scale_y = nn.Parameter(torch.tensor(float(y_scale0)))
        else:
            # If we don't inject y in this mode, keep it as a buffer so DDP doesn't expect grads.
            self.register_buffer("inject_scale_y", torch.tensor(float(y_scale0)))
    ...
    def forward(
        ...
        *,
            ...
            for step in range(self.max_depth_steps):
                ...
                if self.adaptive_halt and (not self.training):
                    ...
                    if prev_eps is not None and steps_taken >= self.min_steps:
                        delta = (eps - prev_eps).abs().mean().item()
        ...
        # Initialize epsilon estimate as zeros in pixel/latent space:
        y_eps_img = torch.zeros(x.shape[0], self.in_channels, x.shape[2], x.shape[3], device=x.device, dtype=x.dtype)
        y_tokens = self._embed_input(y_eps_img)
        ...
        for h in range(self.H_cycles):
            ...
            # Update y_tokens by embedding the current epsilon prediction.
            # (This resolves "x_t -> epsilon" modality mismatch: we bring epsilon
            #  back into token space via PatchEmbed + pos embedding.)
            y_eps_img = eps
            y_tokens = self._embed_input(y_eps_img)
