...
def repa_cosine_loss(student_proj: torch.Tensor, teacher_tok: torch.Tensor) -> torch.Tensor:
    ...
    We compute the cosine in float32 for numerical stability under mixed precision.
    Inputs:
...
def build_repa_mlp(
    ...
    *,
    ...
    """REPA-style projector MLP.

    The official REPA repo uses a small MLP projector (commonly 3 Linear layers
    with SiLU nonlinearity), mapping:

        hidden_size (student) -> projector_dim -> ... -> z_dim

    We implement a configurable 2- or 3-layer variant.
    """
    ...
    if act == "silu":
        ...
        raise ValueError(f"Unknown activation '{act}'. Use silu/gelu/relu.")
...
def attach_repa_projector(
    student: torch.nn.Module,
    teacher_dim: int,
    *,
    use_repa_mlp: bool,
    projector_embed_dims: Optional[int] = None,
    mlp_layers: int = 3,
    mlp_act: str = "silu",
) -> None:
    """Attach a projector module to the student as `student.repa_projector`.

    Design goal:
      - Keep the old *linear* projector path intact (LayerNorm + Linear) for
        backwards compatibility and small overhead.
      - When `use_repa_mlp=True`, switch to an "official REPA style" MLP:
        (Linear -> SiLU -> Linear -> SiLU -> Linear), controlled by
        `--projector-embed-dims` and `--repa-mlp-layers`.
    """
    ...
    if use_repa_mlp:
        hid = int(projector_embed_dims) if projector_embed_dims is not None else int(teacher_dim)
        proj = build_repa_mlp(d_student, hid, int(teacher_dim), num_layers=int(mlp_layers), act=str(mlp_act))
    else:
        # Legacy (small) projector: LN + Linear (matches the previous behavior and
        # explains the small param-count increase you observed).
        proj = torch.nn.Sequential(
            torch.nn.LayerNorm(d_student),
            torch.nn.Linear(d_student, int(teacher_dim)),
        )
...
def main(args):
    ...
    if args.use_repa:
        ...
        if rank == 0:
            dummy = torch.zeros(1, 3, args.repa_input_size, args.repa_input_size, device=device, dtype=(amp_dtype or torch.float32))
        ...
        attach_repa_projector(
            model_base,
            teacher_dim,
            use_repa_mlp=bool(args.repa_proj_mlp),
            projector_embed_dims=args.projector_embed_dims,
            mlp_layers=args.repa_mlp_layers,
            mlp_act=args.repa_mlp_act,
        )

        d_student = int(model_base.pos_embed.shape[-1])
        if args.repa_proj_mlp:
            hid = int(args.projector_embed_dims) if args.projector_embed_dims is not None else int(teacher_dim)
            logger.info(
                "Attached REPA projector (MLP): "
                f"student_dim={d_student} -> hidden={hid} -> z_dim={teacher_dim} "
                f"(layers={args.repa_mlp_layers}, act={args.repa_mlp_act})"
            )
        else:
            logger.info(f"Attached REPA projector (LN+Linear): student_dim={d_student} -> z_dim={teacher_dim}")
        ...
        for x, y in loader:
            ...
            if args.use_repa and repa_state is not None:
                ...
                if args.repa_r_strategy == "fixed":
                    ...
                    if hasattr(student_mod, "trm_mode") and str(student_mod.trm_mode) == "self_refine":
                        max_calls = int(student_mod.H_cycles) * int(student_mod.L_cycles)
                    else:
            ...
            # Diffusion loss (unchanged), under autocast:
            with torch.autocast(device_type="cuda", dtype=amp_dtype, enabled=amp_enabled):
                loss_dict = diffusion.training_losses(model, x_lat, t, model_kwargs)
                loss_diff = loss_dict["loss"].float().mean()
            ...
            if args.use_repa and teacher is not None and repa_state is not None:
                ...
                if lam > 0:
                    ...
                    with torch.autocast(device_type="cuda", dtype=amp_dtype, enabled=amp_enabled):
                        s_proj = unwrap_model(model).repa_projector(z)
                    ...
                    loss_repa = repa_cosine_loss(s_proj, t_tok)
                        ...
                        # Total loss: diffusion + optional REPA.
                        if loss_repa is not None and lam > 0:
                            loss = loss_diff + (lam * loss_repa)
                        else:
                            loss = loss_diff
                            # DDP stability: if REPA early-stops (λ=0), the REPA projector becomes unused
                            # and DDP can error with "Expected to have finished reduction...".
                            # We add a zero-weighted dummy term that touches projector params so they are
                            # always part of the autograd graph (without computing teacher features).
                            if args.use_repa and hasattr(unwrap_model(model), "repa_projector"):
                                proj = unwrap_model(model).repa_projector
                                dummy = torch.zeros((), device=device, dtype=loss_diff.dtype)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--compile", action="store_true",
                        help="Enable torch.compile for the training model (may interact with REPA hooks).")
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Use a REPA-style MLP projector (default: 3 Linear layers with SiLU) instead of the small LN+Linear projector. "
            "This is closer to the official REPA implementation."
        ),
    )
    # Match the official REPA CLI name used in their README generation script.
    parser.add_argument(
        "--projector-embed-dims",
        type=int,
        default=None,
        help=(
            "Hidden dimension of the REPA projector MLP. If None, defaults to teacher_dim (e.g., 768 for dinov2-vit-b). "
            "(REPA README uses --projector-embed-dims=768.)"
        ),
    )
    parser.add_argument(
        "--repa-mlp-layers",
        ...
        default=3,
        choices=[2, 3],
        help="Number of Linear layers in the REPA projector MLP (2 or 3).",
    )
    parser.add_argument(
        "--repa-mlp-act",
        type=str,
        ...
        help="Activation function used in the REPA projector MLP.",
    )
