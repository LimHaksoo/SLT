...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        for x, y in loader:
            ...
            # Default to diffusion loss. (Important: must assign `loss` every iteration;
            # otherwise when REPA turns off, we'd accidentally reuse the previous step's
            # graph and hit "Trying to backward through the graph a second time".)
            loss = loss_diff
            loss_repa = None
            lam = 0.0
            if args.use_repa and teacher is not None and repa_state is not None:
                ...
                if lam > 0:
                    ...
                    # Total loss: diffusion + REPA
                    loss = loss + (lam * loss_repa)
                else:
                    # DDP stability: when REPA is off (λ=0), the REPA projector would be unused.
                    # Touch projector params with a zero-weight term so they stay in the graph
                    # without computing teacher features.
