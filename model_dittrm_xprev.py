...
class DiTTRM(nn.Module):
    ...
    def __init__(
        ...
        **block_kwargs,
        ...
        #  - "eps": feed back epsilon (original TRM-style adaptation in this repo)
        ...
        if self.self_refine_feedback not in ["eps", "xprev", "x0"]:
            raise ValueError(f"Unknown self_refine_feedback={self.self_refine_feedback}. Use one of: eps, xprev, x0")

        # Precompute a default diffusion schedule (DiT defaults: 1000-step linear betas).
        # This is only used when self_refine_feedback != "eps" (to convert eps<->x0/xprev).
    ...
    def forward(
        ...
        *,
        ...
        # Initialize the feedback tensor that will be injected (embedded) each outer cycle.
        # Default ("eps") keeps the previous behavior (feed back eps). For "xprev"/"x0"
        # we start from x_t as a reasonable in-domain initialization.
        if self.self_refine_feedback == "eps":
            ...
            y_img = x  # start from x_t (same latent domain)
        ...
        for h in range(self.H_cycles):
            ...
            # We keep the *diffusion objective* as epsilon prediction, but we can choose to
            # feed back a more "in-domain" latent (x_{t-1} or x0) instead of eps itself.
            if self.self_refine_feedback == "eps":
                y_img = eps
            elif self.self_refine_feedback == "x0":
                y_img = self._predict_x0_from_eps(x, t, eps)
            elif self.self_refine_feedback == "xprev":
                y_img = self._predict_xprev_from_eps(x, t, eps)
            else:
                y_img = eps
            y_tokens = self._embed_input(y_img)
